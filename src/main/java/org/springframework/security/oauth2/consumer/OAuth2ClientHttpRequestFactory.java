...
public class OAuth2ClientHttpRequestFactory implements ClientHttpRequestFactory {
  ...
  protected URI appendQueryParameter(URI uri, OAuth2AccessToken accessToken) {
    try {
      ...
      final String bearerTokenName = (resource.getBearerTokenName() == null) ? "oauth_token" : resource.getBearerTokenName();
      final String oauth_token = bearerTokenName + "=" + URLEncoder.encode(accessToken.getValue(), "UTF-8");
      if (query == null || query.length() == 0) {
        query = oauth_token;
      } else if(query.indexOf(bearerTokenName) == -1) {
        query = query + "&" + oauth_token;
      } // else leave the existing oauth_token parameter in the query alone
